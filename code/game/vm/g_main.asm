...
line 1512
;1511:
;1512:	if ( level.numPlayingClients < 2 ) {
ADDRGP4 level+80
INDIRI4
CNSTI4 2
GEI4 $842
...
;1514:	}
;1515:
;1516:	if ( g_gametype.integer < GT_CTF && g_fraglimit.integer ) {
ADDRGP4 g_gametype+12
INDIRI4
CNSTI4 7
GEI4 $845
...
EQI4 $845
line 1517
;1517:		if ( level.teamScores[TEAM_RED] >= g_fraglimit.integer ) {
ADDRGP4 level+44+4
INDIRI4
ADDRGP4 g_fraglimit+12
INDIRI4
...
;1521:		}
;1522:
;1523:		if ( level.teamScores[TEAM_BLUE] >= g_fraglimit.integer ) {
ADDRGP4 level+44+8
INDIRI4
ADDRGP4 g_fraglimit+12
INDIRI4
LTI4 $857
line 1524
;1524:			trap_SendServerCommand( -1, va("print \"Blue %s\n\"", G_GetStripEdString("SVINGAME", "HIT_THE_KILL_LIMIT")) );
ADDRGP4 $839
ARGP4
ADDRGP4 $855
ARGP4
...
;1527:		}
;1528:
;1529:		for ( i=0 ; i< g_maxclients.integer ; i++ ) {
ADDRLP4 4
CNSTI4 0
ASGNI4
ADDRGP4 $866
...
ASGNP4
line 1531
;1531:			if ( cl->pers.connected != CON_CONNECTED ) {
ADDRLP4 0
INDIRP4
CNSTI4 1368
ADDP4
...
line 1534
;1533:			}
;1534:			if ( cl->sess.sessionTeam != TEAM_FREE ) {
ADDRLP4 0
INDIRP4
CNSTI4 1520
ADDP4
...
;1536:			}
;1537:
;1538:			if ( g_gametype.integer == GT_TOURNAMENT && g_duel_fraglimit.integer && cl->sess.wins >= g_duel_fraglimit.integer )
ADDRGP4 g_gametype+12
INDIRI4
CNSTI4 3
NEI4 $872
...
pop
line 1541
;1541:				gDuelExit = qtrue;
ADDRGP4 gDuelExit
CNSTI4 1
ASGNI4
line 1542
;1542:				trap_SendServerCommand( -1, va("print \"%s" S_COLOR_WHITE " hit the win limit.\n\"",
ADDRGP4 $878
ARGP4
ADDRLP4 0
INDIRP4
...
;1545:			}
;1546:
;1547:			if ( cl->ps.persistant[PERS_SCORE] >= g_fraglimit.integer ) {
ADDRLP4 0
INDIRP4
CNSTI4 280
ADDP4
...
pop
line 1549
;1549:				gDuelExit = qfalse;
ADDRGP4 gDuelExit
CNSTI4 0
ASGNI4
line 1550
;1550:				trap_SendServerCommand( -1, va("print \"%s" S_COLOR_WHITE " %s.\n\"",
ADDRGP4 $839
ARGP4
ADDRGP4 $855
ARGP4
...
pop
line 1555
;1551:												cl->pers.netname,
;1552:												G_GetStripEdString("SVINGAME", "HIT_THE_KILL_LIMIT")
;1553:												) 
;1554:										);
;1555:				return;
ADDRGP4 $811
...
line 1560
;1559:
;1560:	if ( g_gametype.integer >= GT_CTF && g_capturelimit.integer ) {
ADDRGP4 g_gametype+12
INDIRI4
CNSTI4 7
LTI4 $883
...
line 1562
;1561:
;1562:		if ( level.teamScores[TEAM_RED] >= g_capturelimit.integer ) {
ADDRGP4 level+44+4
INDIRI4
ADDRGP4 g_capturelimit+12
INDIRI4
LTI4 $887
line 1563
;1563:			trap_SendServerCommand( -1, "print \"Red hit the capturelimit.\n\"" );
CNSTI4 -1
ARGI4
ADDRGP4 $892
ARGP4
...
;1566:		}
;1567:
;1568:		if ( level.teamScores[TEAM_BLUE] >= g_capturelimit.integer ) {
ADDRGP4 level+44+8
INDIRI4
ADDRGP4 g_capturelimit+12
INDIRI4
LTI4 $894
line 1569
;1569:			trap_SendServerCommand( -1, "print \"Blue hit the capturelimit.\n\"" );
CNSTI4 -1
ARGI4
ADDRGP4 $899
ARGP4
...
;1579:========================================================================
;1580:
;1581:FUNCTIONS CALLED EVERY FRAME
;1582:
;1583:========================================================================
;1584:*/
;1585:
;1586:/*
;1587:=============
;1588:CheckTournament
;1589:
;1590:Once a frame, check for changes in tournement player state
;1591:=============
;1592:*/
;1593:void CheckTournament( void ) {
line 1596
;1594:	// check because we run 3 game frames before calling Connect and/or ClientBegin
;1595:	// for clients on a map_restart
;1596:	if ( level.numPlayingClients == 0 ) {
ADDRGP4 level+80
INDIRI4
CNSTI4 0
NEI4 $901
...
;1598:	}
;1599:
;1600:	if ( g_gametype.integer == GT_TOURNAMENT ) {
ADDRGP4 g_gametype+12
INDIRI4
CNSTI4 3
NEI4 $904
line 1603
;1601:
;1602:		// pull in a spectator if needed
;1603:		if ( level.numPlayingClients < 2 ) {
ADDRGP4 level+80
INDIRI4
CNSTI4 2
GEI4 $907
line 1604
;1604:			AddTournamentPlayer();
ADDRGP4 AddTournamentPlayer
CALLV
pop
line 1606
;1605:
;1606:			if (level.numPlayingClients >= 2)
ADDRGP4 level+80
INDIRI4
CNSTI4 2
LTI4 $910
...
;1607:			{
line 1608
;1608:				trap_SetConfigstring ( CS_CLIENT_DUELISTS, va("%i|%i", level.sortedClients[0], level.sortedClients[1] ) );
ADDRGP4 $735
ARGP4
ADDRGP4 level+84
INDIRI4
...
pop
line 1609
;1609:				gDuelist1 = level.sortedClients[0];
ADDRGP4 gDuelist1
ADDRGP4 level+84
INDIRI4
ASGNI4
line 1610
;1610:				gDuelist2 = level.sortedClients[1];
ADDRGP4 gDuelist2
ADDRGP4 level+84+4
INDIRI4
ASGNI4
...
;1618:			{
line 1619
;1619:				trap_SetConfigstring ( CS_CLIENT_DUELISTS, va("%i|%i", level.sortedClients[0], level.sortedClients[1] ) );
ADDRGP4 $735
ARGP4
ADDRGP4 level+84
INDIRI4
...
pop
line 1620
;1620:				gDuelist1 = level.sortedClients[0];
ADDRGP4 gDuelist1
ADDRGP4 level+84
INDIRI4
ASGNI4
line 1621
;1621:				gDuelist2 = level.sortedClients[1];
ADDRGP4 gDuelist2
ADDRGP4 level+84+4
INDIRI4
ASGNI4
...
line 1627
;1624:
;1625:		//rww - It seems we have decided there will be no warmup in duel.
;1626:		//if (!g_warmup.integer)
;1627:		{ //don't care about any of this stuff then, just add people and leave me alone
line 1628
;1628:			level.warmupTime = 0;
ADDRGP4 level+16
CNSTI4 0
ASGNI4
line 1629
...
;1630:		}
;1631:#if 0
;1632:		// if we don't have two players, go back to "waiting for players"
;1633:		if ( level.numPlayingClients != 2 ) {
...
;1654:			if ( level.numPlayingClients == 2 ) {
;1655:				// fudge by -1 to account for extra delays
;1656:				level.warmupTime = level.time + ( g_warmup.integer - 1 ) * 1000;
;1657:
;1658:				if (level.warmupTime < (level.time + 3000))
;1659:				{ //rww - this is an unpleasent hack to keep the level from resetting completely on the client (this happens when two map_restarts are issued rapidly)
;1660:					level.warmupTime = level.time + 3000;
;1661:				}
;1662:				trap_SetConfigstring( CS_WARMUP, va("%i", level.warmupTime) );
;1663:			}
;1664:			return;
;1665:		}
;1666:
;1667:		// if the warmup time has counted down, restart
;1668:		if ( level.time > level.warmupTime ) {
;1669:			level.warmupTime += 10000;
;1670:			trap_Cvar_Set( "g_restarted", "1" );
;1671:			trap_SendConsoleCommand( EXEC_APPEND, "map_restart 0\n" );
;1672:			level.restarted = qtrue;
;1673:			return;
;1674:		}
;1675:#endif
;1676:	} else if ( g_gametype.integer != GT_SINGLE_PLAYER && level.warmupTime != 0 ) {
ADDRGP4 g_gametype+12
INDIRI4
CNSTI4 4
EQI4 $932
...
EQI4 $932
line 1678
;1677:		int		counts[TEAM_NUM_TEAMS];
;1678:		qboolean	notEnough = qfalse;
ADDRLP4 0
CNSTI4 0
ASGNI4
line 1680
;1679:
;1680:		if ( g_gametype.integer > GT_TEAM ) {
ADDRGP4 g_gametype+12
INDIRI4
CNSTI4 5
LEI4 $936
line 1681
;1681:			counts[TEAM_BLUE] = TeamCount( -1, TEAM_BLUE );
CNSTI4 -1
ARGI4
CNSTI4 2
ARGI4
...
ASGNI4
line 1682
;1682:			counts[TEAM_RED] = TeamCount( -1, TEAM_RED );
CNSTI4 -1
ARGI4
CNSTI4 1
ARGI4
...
line 1684
;1683:
;1684:			if (counts[TEAM_RED] < 1 || counts[TEAM_BLUE] < 1) {
ADDRLP4 28
CNSTI4 1
ASGNI4
ADDRLP4 4+4
...
;1686:			}
line 1687
;1687:		} else if ( level.numPlayingClients < 2 ) {
ADDRGP4 $937
JUMPV
LABELV $936
ADDRGP4 level+80
...
EQI4 $949
line 1692
;1692:			if ( level.warmupTime != -1 ) {
ADDRGP4 level+16
INDIRI4
CNSTI4 -1
EQI4 $900
line 1693
;1693:				level.warmupTime = -1;
ADDRGP4 level+16
CNSTI4 -1
ASGNI4
line 1694
;1694:				trap_SetConfigstring( CS_WARMUP, va("%i", level.warmupTime) );
ADDRGP4 $375
ARGP4
ADDRGP4 level+16
INDIRI4
...
;1696:			}
line 1697
;1697:			return; // still waiting for team members
ADDRGP4 $900
JUMPV
LABELV $949
line 1700
...
;1702:		}
;1703:
;1704:		// if the warmup is changed at the console, restart it
;1705:		if ( g_warmup.modificationCount != level.warmupModificationCount ) {
ADDRGP4 g_warmup+4
INDIRI4
ADDRGP4 level+224
INDIRI4
EQI4 $960
line 1706
;1706:			level.warmupModificationCount = g_warmup.modificationCount;
ADDRGP4 level+224
ADDRGP4 g_warmup+4
INDIRI4
ASGNI4
line 1707
;1707:			level.warmupTime = -1;
ADDRGP4 level+16
CNSTI4 -1
ASGNI4
line 1708
...
line 1711
;1709:
;1710:		// if all players have arrived, start the countdown
;1711:		if ( level.warmupTime < 0 ) {
ADDRGP4 level+16
INDIRI4
CNSTI4 0
GEI4 $967
line 1713
;1712:			// fudge by -1 to account for extra delays
;1713:			level.warmupTime = level.time + ( g_warmup.integer - 1 ) * 1000;
ADDRLP4 20
CNSTI4 1000
ASGNI4
ADDRGP4 level+16
...
ASGNI4
line 1714
;1714:			trap_SetConfigstring( CS_WARMUP, va("%i", level.warmupTime) );
ADDRGP4 $375
ARGP4
ADDRGP4 level+16
INDIRI4
...
;1716:		}
;1717:
;1718:		// if the warmup time has counted down, restart
;1719:		if ( level.time > level.warmupTime ) {
ADDRGP4 level+32
INDIRI4
ADDRGP4 level+16
INDIRI4
...
ASGNI4
line 1721
;1721:			trap_Cvar_Set( "g_restarted", "1" );
ADDRGP4 $85
ARGP4
ADDRGP4 $101
ARGP4
...
pop
line 1722
;1722:			trap_SendConsoleCommand( EXEC_APPEND, "map_restart 0\n" );
CNSTI4 2
ARGI4
ADDRGP4 $613
ARGP4
...
;1733:==================
;1734:*/
;1735:void CheckVote( void ) {
line 1736
;1736:	if ( level.voteExecuteTime && level.voteExecuteTime < level.time ) {
ADDRGP4 level+2280
INDIRI4
CNSTI4 0
EQI4 $981
...
GEI4 $981
line 1737
;1737:		level.voteExecuteTime = 0;
ADDRGP4 level+2280
CNSTI4 0
ASGNI4
line 1738
;1738:		trap_SendConsoleCommand( EXEC_APPEND, va("%s\n", level.voteString ) );
ADDRGP4 $987
ARGP4
ADDRGP4 level+228
ARGP4
...
;1741:		{
line 1742
;1742:			if (trap_Cvar_VariableIntegerValue("g_gametype") != level.votingGametypeTo)
ADDRGP4 $88
ARGP4
ADDRLP4 4
ADDRGP4 trap_Cvar_VariableIntegerValue
...
EQI4 $992
line 1743
;1743:			{ //If we're voting to a different game type, be sure to refresh all the map stuff
line 1744
;1744:				const char *nextMap = G_RefreshNextMap(level.votingGametypeTo, qtrue);
ADDRGP4 level+2300
INDIRI4
ARGI4
CNSTI4 1
...
;1747:				{
line 1748
;1748:					trap_SendConsoleCommand( EXEC_APPEND, va("map %s\n", nextMap ) );
ADDRGP4 $998
ARGP4
ADDRLP4 8
INDIRP4
...
line 1753
;1752:			else
;1753:			{ //otherwise, just leave the map until a restart
line 1754
;1754:				G_RefreshNextMap(level.votingGametypeTo, qfalse);
ADDRGP4 level+2300
INDIRI4
ARGI4
CNSTI4 0
...
LABELV $993
line 1756
;1756:			level.votingGametype = qfalse;
ADDRGP4 level+2296
CNSTI4 0
ASGNI4
line 1757
;1757:			level.votingGametypeTo = 0;
ADDRGP4 level+2300
CNSTI4 0
ASGNI4
line 1758
...
line 1763
;1762:	}
;1763:	if ( level.time - level.voteTime >= VOTE_TIME ) {
ADDRGP4 level+32
INDIRI4
ADDRGP4 level+2276
INDIRI4
...
LTI4 $1005
line 1764
;1764:		trap_SendServerCommand( -1, va("print \"%s\n\"", G_GetStripEdString("SVINGAME", "VOTEFAILED")) );
ADDRGP4 $839
ARGP4
ADDRGP4 $1010
ARGP4
...
LABELV $1005
line 1766
;1766:		if ( level.voteYes > level.numVotingClients/2 ) {
ADDRGP4 level+2284
INDIRI4
ADDRGP4 level+2292
INDIRI4
...
LEI4 $1011
line 1768
;1767:			// execute the command, then remove the vote
;1768:			trap_SendServerCommand( -1, va("print \"%s\n\"", G_GetStripEdString("SVINGAME", "VOTEPASSED")) );
ADDRGP4 $839
ARGP4
ADDRGP4 $1015
ARGP4
...
pop
line 1769
;1769:			level.voteExecuteTime = level.time + 3000;
ADDRGP4 level+2280
ADDRGP4 level+32
INDIRI4
CNSTI4 3000
...
ASGNI4
line 1770
;1770:		} else if ( level.voteNo >= level.numVotingClients/2 ) {
ADDRGP4 $1012
JUMPV
LABELV $1011
ADDRGP4 level+2288
...
LTI4 $980
line 1772
;1771:			// same behavior as a timeout
;1772:			trap_SendServerCommand( -1, va("print \"%s\n\"", G_GetStripEdString("SVINGAME", "VOTEFAILED")) );
ADDRGP4 $839
ARGP4
ADDRGP4 $1010
ARGP4
...
;1773:		} else {
line 1775
;1774:			// still waiting for a majority
;1775:			return;
LABELV $1019
LABELV $1012
line 1777
...
LABELV $1006
line 1778
;1778:	level.voteTime = 0;
ADDRGP4 level+2276
CNSTI4 0
ASGNI4
line 1779
;1779:	trap_SetConfigstring( CS_VOTE_TIME, "" );
CNSTI4 8
ARGI4
ADDRGP4 $80
ARGP4
...
;1786:==================
;1787:*/
;1788:void PrintTeam(int team, char *message) {
line 1791
;1789:	int i;
;1790:
;1791:	for ( i = 0 ; i < level.maxclients ; i++ ) {
ADDRLP4 0
CNSTI4 0
ASGNI4
ADDRGP4 $1027
...
LABELV $1024
line 1792
;1792:		if (level.clients[i].sess.sessionTeam != team)
CNSTI4 1756
ADDRLP4 0
INDIRI4
MULI4
...
LABELV $1029
line 1794
;1794:		trap_SendServerCommand( i, message );
ADDRLP4 0
INDIRI4
ARGI4
ADDRFP4 4
...
;1801:==================
;1802:*/
;1803:void SetLeader(int team, int client) {
line 1806
;1804:	int i;
;1805:
;1806:	if ( level.clients[client].pers.connected == CON_DISCONNECTED ) {
CNSTI4 1756
ADDRFP4 4
INDIRI4
MULI4
...
NEI4 $1032
line 1807
;1807:		PrintTeam(team, va("print \"%s is not connected\n\"", level.clients[client].pers.netname) );
ADDRGP4 $1034
ARGP4
CNSTI4 1756
ADDRFP4 4
...
line 1810
;1809:	}
;1810:	if (level.clients[client].sess.sessionTeam != team) {
CNSTI4 1756
ADDRFP4 4
INDIRI4
MULI4
...
EQI4 $1035
line 1811
;1811:		PrintTeam(team, va("print \"%s is not on the team anymore\n\"", level.clients[client].pers.netname) );
ADDRGP4 $1037
ARGP4
CNSTI4 1756
ADDRFP4 4
...
line 1814
;1813:	}
;1814:	for ( i = 0 ; i < level.maxclients ; i++ ) {
ADDRLP4 0
CNSTI4 0
ASGNI4
ADDRGP4 $1041
...
LABELV $1038
line 1815
;1815:		if (level.clients[i].sess.sessionTeam != team)
CNSTI4 1756
ADDRLP4 0
INDIRI4
MULI4
...
LABELV $1043
line 1817
;1817:		if (level.clients[i].sess.teamLeader) {
CNSTI4 1756
ADDRLP4 0
INDIRI4
MULI4
...
EQI4 $1045
line 1818
;1818:			level.clients[i].sess.teamLeader = qfalse;
CNSTI4 1756
ADDRLP4 0
INDIRI4
MULI4
...
LTI4 $1038
line 1822
;1822:	level.clients[client].sess.teamLeader = qtrue;
CNSTI4 1756
ADDRFP4 4
INDIRI4
MULI4
...
ASGNI4
line 1823
;1823:	ClientUserinfoChanged( client );
ADDRFP4 4
INDIRI4
ARGI4
ADDRGP4 ClientUserinfoChanged
...
;1827:/*
;1828:==================
;1829:CheckTeamLeader
;1830:==================
;1831:*/
;1832:void CheckTeamLeader( int team ) {
line 1835
;1833:	int i;
;1834:
;1835:	for ( i = 0 ; i < level.maxclients ; i++ ) {
ADDRLP4 0
CNSTI4 0
ASGNI4
ADDRGP4 $1053
...
LABELV $1050
line 1836
;1836:		if (level.clients[i].sess.sessionTeam != team)
CNSTI4 1756
ADDRLP4 0
INDIRI4
MULI4
...
LABELV $1055
line 1838
;1838:		if (level.clients[i].sess.teamLeader)
CNSTI4 1756
ADDRLP4 0
INDIRI4
MULI4
...
LABELV $1062
line 1843
;1843:			if (level.clients[i].sess.sessionTeam != team)
CNSTI4 1756
ADDRLP4 0
INDIRI4
MULI4
...
LABELV $1067
line 1845
;1845:			if (!(g_entities[i].r.svFlags & SVF_BOT)) {
CNSTI4 828
ADDRLP4 0
INDIRI4
MULI4
...
NEI4 $1069
line 1846
;1846:				level.clients[i].sess.teamLeader = qtrue;
CNSTI4 1756
ADDRLP4 0
INDIRI4
MULI4
...
LABELV $1073
line 1851
;1851:			if (level.clients[i].sess.sessionTeam != team)
CNSTI4 1756
ADDRLP4 0
INDIRI4
MULI4
...
LABELV $1078
line 1853
;1853:			level.clients[i].sess.teamLeader = qtrue;
CNSTI4 1756
ADDRLP4 0
INDIRI4
MULI4
...
;1862:==================
;1863:*/
;1864:void CheckTeamVote( int team ) {
line 1867
;1865:	int cs_offset;
;1866:
;1867:	if ( team == TEAM_RED )
ADDRFP4 0
INDIRI4
CNSTI4 1
NEI4 $1081
...
line 1874
;1873:
;1874:	if ( !level.teamVoteTime[cs_offset] ) {
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
...
line 1877
;1876:	}
;1877:	if ( level.time - level.teamVoteTime[cs_offset] >= VOTE_TIME ) {
ADDRGP4 level+32
INDIRI4
ADDRLP4 0
INDIRI4
...
LABELV $1088
line 1880
;1880:		if ( level.teamVoteYes[cs_offset] > level.numteamVotingClients[cs_offset]/2 ) {
ADDRLP4 4
CNSTI4 2
ASGNI4
ADDRLP4 8
...
LEI4 $1093
line 1882
;1881:			// execute the command, then remove the vote
;1882:			trap_SendServerCommand( -1, va("print \"%s\n\"", G_GetStripEdString("SVINGAME", "TEAMVOTEPASSED")) );
ADDRGP4 $839
ARGP4
ADDRGP4 $1097
ARGP4
...
line 1884
;1883:			//
;1884:			if ( !Q_strncmp( "leader", level.teamVoteString[cs_offset], 6) ) {
ADDRGP4 $1100
ARGP4
ADDRLP4 0
INDIRI4
...
NEI4 $1098
line 1886
;1885:				//set the team leader
;1886:				SetLeader(team, atoi(level.teamVoteString[cs_offset] + 7));
ADDRLP4 0
INDIRI4
CNSTI4 10
LSHI4
...
;1888:			else {
line 1889
;1889:				trap_SendConsoleCommand( EXEC_APPEND, va("%s\n", level.teamVoteString[cs_offset] ) );
ADDRGP4 $987
ARGP4
ADDRLP4 0
INDIRI4
...
;1890:			}
line 1891
;1891:		} else if ( level.teamVoteNo[cs_offset] >= level.numteamVotingClients[cs_offset]/2 ) {
ADDRGP4 $1094
JUMPV
LABELV $1093
ADDRLP4 12
...
LTI4 $1080
line 1893
;1892:			// same behavior as a timeout
;1893:			trap_SendServerCommand( -1, va("print \"%s\n\"", G_GetStripEdString("SVINGAME", "TEAMVOTEFAILED")) );
ADDRGP4 $839
ARGP4
ADDRGP4 $1092
ARGP4
...
;1894:		} else {
line 1896
;1895:			// still waiting for a majority
;1896:			return;
LABELV $1106
LABELV $1094
line 1898
...
LABELV $1089
line 1899
;1899:	level.teamVoteTime[cs_offset] = 0;
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
...
ASGNI4
line 1900
;1900:	trap_SetConfigstring( CS_TEAMVOTE_TIME + cs_offset, "" );
ADDRLP4 0
INDIRI4
CNSTI4 12
ADDI4
...
;1908:==================
;1909:*/
;1910:void CheckCvars( void ) {
line 1913
;1911:	static int lastMod = -1;
;1912:
;1913:	if ( g_password.modificationCount != lastMod ) {
...
EQI4 $1112
line 1914
;1914:		lastMod = g_password.modificationCount;
ADDRGP4 $1111
ADDRGP4 g_password+4
INDIRI4
ASGNI4
line 1915
;1915:		if ( *g_password.string && Q_stricmp( g_password.string, "none" ) ) {
...
EQI4 $1116
line 1916
;1916:			trap_Cvar_Set( "g_needpass", "1" );
ADDRGP4 $133
ARGP4
ADDRGP4 $101
ARGP4
...
LABELV $1116
line 1918
;1918:			trap_Cvar_Set( "g_needpass", "0" );
ADDRGP4 $133
ARGP4
ADDRGP4 $86
ARGP4
...
;1925:G_RunThink
;1926:
;1927:Runs thinking code for this frame if necessary
;1928:=============
;1929:*/
;1930:void G_RunThink (gentity_t *ent) {
line 1933
;1931:	float	thinktime;
;1932:
;1933:	thinktime = ent->nextthink;
ADDRLP4 0
ADDRFP4 0
INDIRP4
CNSTI4 632
...
line 1959
;1947:
;1948:int g_LastFrameTime = 0;
;1949:int g_TimeSinceLastFrame = 0;
;1950:
;1951:/*
;1952:================
;1953:G_RunFrame
;1954:
;1955:Advances the non-player objects in the world
;1956:================
;1957:*/
;1958:
;1959:void G_RunFrame( int levelTime ) {
line 1966
;1960:	int			i;
;1961:	gentity_t	*ent;
;1962:	int			msec;
;1963:int start, end;
;1964:
;1965:	// if we are waiting for the level to restart, do nothing
;1966:	if ( level.restarted ) {
ADDRGP4 level+68
INDIRI4
CNSTI4 0
EQI4 $1131
...
ASGNI4
line 1971
;1971:	level.previousTime = level.time;
ADDRGP4 level+36
ADDRGP4 level+32
INDIRI4
ASGNI4
line 1972
;1972:	level.time = levelTime;
ADDRGP4 level+32
ADDRFP4 0
INDIRI4
ASGNI4
line 1973
;1973:	msec = level.time - level.previousTime;
ADDRLP4 16
ADDRGP4 level+32
INDIRI4
ADDRGP4 level+36
...
line 1975
;1974:
;1975:	g_TimeSinceLastFrame = (level.time - g_LastFrameTime);
...
;1979:
;1980:	//
;1981:	// go through all allocated objects
;1982:	//
;1983:	start = trap_Milliseconds();
ADDRLP4 24
...
ASGNI4
line 1984
;1984:	ent = &g_entities[0];
ADDRLP4 0
ADDRGP4 g_entities
ASGNP4
line 1985
;1985:	for (i=0 ; i<level.num_entities ; i++, ent++) {
ADDRLP4 4
CNSTI4 0
ASGNI4
ADDRGP4 $1144
...
;1988:		}
;1989:
;1990:		// clear events that are too old
;1991:		if ( level.time - ent->eventTime > EVENT_VALID_MSEC ) {
ADDRGP4 level+32
INDIRI4
ADDRLP4 0
INDIRP4
...
EQI4 $1151
line 1993
;1993:				ent->s.event = 0;	// &= EV_EVENT_BITS;
ADDRLP4 0
INDIRP4
CNSTI4 252
ADDP4
...
EQU4 $1153
line 1995
;1995:					ent->client->ps.externalEvent = 0;
ADDRLP4 0
INDIRP4
CNSTI4 408
ADDP4
...
ASGNI4
line 1999
;1996:					// predicted events should never be set to zero
;1997:					//ent->client->ps.events[0] = 0;
;1998:					//ent->client->ps.events[1] = 0;
;1999:				}
LABELV $1153
line 2000
;2000:			}
...
EQI4 $1155
line 2003
;2002:				// tempEntities or dropped items completely go away after their event
;2003:				if (ent->s.eFlags & EF_SOUNDTRACKER)
ADDRLP4 0
INDIRP4
CNSTI4 8
ADDP4
...
EQI4 $1157
line 2004
;2004:				{ //don't trigger the event again..
line 2005
;2005:					ent->s.event = 0;
ADDRLP4 0
INDIRP4
...
line 2015
;2014:				}
;2015:			} else if ( ent->unlinkAfterEvent ) {
LABELV $1155
ADDRLP4 0
INDIRP4
CNSTI4 496
...
EQI4 $1159
line 2017
;2016:				// items that will respawn will hide themselves after their pickup event
;2017:				ent->unlinkAfterEvent = qfalse;
ADDRLP4 0
INDIRP4
CNSTI4 496
ADDP4
...
line 2023
;2021:
;2022:		// temporary entities don't think
;2023:		if ( ent->freeAfterEvent ) {
ADDRLP4 0
INDIRP4
CNSTI4 492
ADDP4
...
;2025:		}
;2026:
;2027:		if ( !ent->r.linked && ent->neverFree ) {
ADDRLP4 32
CNSTI4 0
ASGNI4
ADDRLP4 0
...
;2029:		}
;2030:
;2031:		if ( ent->s.eType == ET_MISSILE ) {
ADDRLP4 0
INDIRP4
CNSTI4 4
ADDP4
...
;2034:		}
;2035:
;2036:		if ( ent->s.eType == ET_ITEM || ent->physicsObject ) {
ADDRLP4 0
INDIRP4
CNSTI4 4
ADDP4
...
;2047:		{
line 2048
;2048:			G_CheckClientTimeouts ( ent );
ADDRLP4 0
INDIRP4
ARGP4
...
line 2050
;2049:			
;2050:			if((!level.intermissiontime)&&!(ent->client->ps.pm_flags&PMF_FOLLOW))
ADDRLP4 40
CNSTI4 0
ASGNI4
ADDRGP4 level+9008
...
;2051:			{
line 2052
;2052:				WP_ForcePowersUpdate(ent, &ent->client->pers.cmd );
ADDRLP4 0
INDIRP4
ARGP4
ADDRLP4 0
...
pop
line 2053
;2053:				WP_SaberPositionUpdate(ent, &ent->client->pers.cmd);
ADDRLP4 0
INDIRP4
ARGP4
ADDRLP4 0
...
line 2063
;2062:
;2063:	trap_ROFF_UpdateEntities();
ADDRGP4 trap_ROFF_UpdateEntities
CALLV
pop
line 2065
;2064:
;2065:start = trap_Milliseconds();
ADDRLP4 32
ADDRGP4 trap_Milliseconds
CALLI4
ASGNI4
...
ASGNI4
line 2067
;2066:	// perform final fixups on the players
;2067:	ent = &g_entities[0];
ADDRLP4 0
ADDRGP4 g_entities
ASGNP4
line 2068
;2068:	for (i=0 ; i < level.maxclients ; i++, ent++ ) {
ADDRLP4 4
CNSTI4 0
ASGNI4
ADDRGP4 $1180
...
line 2076
;2074:
;2075:	// see if it is time to do a tournement restart
;2076:	CheckTournament();
ADDRGP4 CheckTournament
CALLV
pop
line 2079
;2077:
;2078:	// see if it is time to end the level
;2079:	CheckExitRules();
ADDRGP4 CheckExitRules
CALLV
pop
line 2082
;2080:
;2081:	// update to team status?
;2082:	CheckTeamStatus();
ADDRGP4 CheckTeamStatus
CALLV
pop
line 2085
;2083:
;2084:	// cancel vote if timed out
;2085:	CheckVote();
ADDRGP4 CheckVote
CALLV
pop
line 2088
;2086:
;2087:	// check team votes
;2088:	CheckTeamVote( TEAM_RED );
CNSTI4 1
ARGI4
...
line 2094
;2093:
;2094:	if (g_listEntity.integer) {
ADDRGP4 g_listEntity+12
INDIRI4
CNSTI4 0
EQI4 $1184
line 2095
;2095:		for (i = 0; i < MAX_GENTITIES; i++) {
ADDRLP4 4
CNSTI4 0
ASGNI4
LABELV $1187
...
LTI4 $1187
line 2098
;2098:		trap_Cvar_Set("g_listEntity", "0");
ADDRGP4 $159
ARGP4
ADDRGP4 $86
ARGP4
...
line 2102
;2100:
;2101:	//At the end of the frame, send out the ghoul2 kill queue, if there is one
;2102:	G_SendG2KillQueue();
ADDRGP4 G_SendG2KillQueue
CALLV
pop
line 2104
;2103:
;2104:	g_LastFrameTime = level.time;
ADDRGP4 g_LastFrameTime
ADDRGP4 level+32
INDIRI4
ASGNI4
...
line 2108
;2106:
;2107:const char *G_GetStripEdString(char *refSection, char *refName)
;2108:{
line 2121
;2109:	/*
;2110:	static char text[1024]={0};
;2111:	trap_SP_GetStringTextString(va("%s_%s", refSection, refName), text, sizeof(text));
;2112:	return text;
;2113:	*/
;2114:
;2115:	//Well, it would've been lovely doing it the above way, but it would mean mixing
;2116:	//languages for the client depending on what the server is. So we'll mark this as
;2117:	//a striped reference with @@@ and send the refname to the client, and when it goes
;2118:	//to print it will get scanned for the striped reference indication and dealt with
;2119:	//properly.
;2120:	static char text[1024]={0};
;2121:	Com_sprintf(text, sizeof(text), "@@@%s", refName);
ADDRGP4 $1195
ARGP4
CNSTI4 1024
ARGI4
